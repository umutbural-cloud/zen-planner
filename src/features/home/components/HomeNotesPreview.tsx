...
const HomeNotesPreview = ({ notes }: Props) => {
  return (
    <section className="rounded-sm border border-border/60 bg-transparent overflow-hidden">
      ...
      {notes.status === "loading" && <div className="mx-4 mb-4 h-24 rounded-sm border border-border/60 bg-transparent animate-pulse" />}
      ...
      {notes.status === "ready" && (
        <ul className="px-2 pb-2 divide-y divide-border/40">
          {notes.data.map((note) => (
            <li key={note.id} className="px-3 py-2 rounded-sm hover:bg-accent/20 transition-colors">
