...
const HomeRecentWorkPreview = ({ recentWork }: Props) => {
  ...
  return (
    <section className="rounded-2xl border border-border/60 bg-card/60 backdrop-blur-sm overflow-hidden">
      ...
      {recentWork.status === "loading" && <div className="mx-4 mb-4 h-24 rounded-xl bg-muted/40 animate-pulse" />}
      ...
      {recentWork.status === "ready" && items.length > 0 && (
        <ul className="px-2 pb-2 divide-y divide-border/40">
          {items.map((item) => (
            <li key={item.id} className="px-3 py-2 rounded-md hover:bg-accent/30 transition-colors">
