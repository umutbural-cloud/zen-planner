...
BEGIN
  ...
  VALUES
    (NEW.id, 'İş', 'blue', 0),
    (NEW.id, 'Yaşam', 'green', 1),
    (NEW.id, 'Dinlenme', 'yellow', 2)
  ...
  VALUES
    (NEW.id, 'Bir bardak su iç', 'circle', 'daily', '{}'::int[], 'morning', 0),
    (NEW.id, 'Nefes egzersizi yap', 'circle', 'daily', '{}'::int[], 'noon', 1),
    (NEW.id, 'Faydalı içerik tüket', 'circle', 'daily', '{}'::int[], 'ikindi', 2),
    (NEW.id, 'Günlük yaz', 'circle', 'daily', '{}'::int[], 'evening', 3),
    (NEW.id, 'Meditasyon yap', 'circle', 'daily', '{}'::int[], 'night', 4);
